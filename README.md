...
at [http://localhost:8080/swagger-ui/index.html](http://localhost:8080/swagger-ui/index.html) after starting the
application.
...
After each operation, an asynchronous task is created, which compares the new person data with the previous version (if
any).  
...
Each field (name, surname, birthDate, company) is compared using text similarity. The similarity is calculated
...
- Use **Apache Kafka** for task processing - the application could publish task events to a Kafka topic (e.g. '
  person-tasks'). And a separate consumer service would then asynchronously process these events in the background.
- Add **pagination** support for listing tasks and persons.
